...
export function normalizeMappingEvidence(input = {}) {
  ...
  const pagesOrSheets = input.pagesOrSheets.map((part, index) => {
    ...
    for (const line of part.lines) {
      ...
      const normalized = { text }; if (Number.isFinite(Number(line.x))) normalized.x = Number(line.x); if (Number.isFinite(Number(line.y))) normalized.y = Number(line.y); if (Number.isFinite(Number(line.fontSize))) normalized.fontSize = Number(line.fontSize); if (typeof line.bold === "boolean") normalized.bold = line.bold; if (Number.isInteger(Number(line.order))) normalized.order = Number(line.order); lines.push(normalized);
    }
