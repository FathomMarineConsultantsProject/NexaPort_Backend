...
export const sendDailyReportError = (res, error, fallbackCode = "DAILY_REPORT_ERROR") => {
...
const sendError = (res, error) => sendDailyReportError(res, error);
...
export const generate = async (req, res) => { try { res.json({ success: true, message: "Daily Report PDF generated", data: await generateDailyReport({ requestId: req.params.requestId, dailyReportId: req.params.dailyReportId, actorUserId: req.user.id }) }); } catch (error) { sendDailyReportError(res, error, "DAILY_REPORT_GENERATION_FAILED"); } };
