...
async function reportForAccess(queryable, reportId, user, { ownerOnly = false } = {}) {
  ...
  const result = await queryable.query(`SELECT r.*,t.title AS template_title,t.source_type,t.source_s3_key,u.full_name AS consultant_name,u.email AS consultant_email,v.fields_jsonb,v.layout_jsonb,v.version_number FROM inspection_reports r JOIN inspection_templates t ON t.id=r.template_id JOIN inspection_template_versions v ON v.id=r.template_version_id JOIN experts e ON e.id=r.expert_id JOIN users u ON u.id=e.user_id WHERE r.id=$1${expertId ? " AND r.expert_id=$2" : ""}`, expertId ? [reportId, expertId] : [reportId]);
  if (!result.rows[0]) throw Object.assign(new Error("Report not found or access denied."), { status: 404 });
  return result.rows[0];
}

const publicReport = ({ source_s3_key, generated_pdf_s3_key, ...row }) => ({ ...row, generated: Boolean(generated_pdf_s3_key) });

export const createReport = async (req, res) => {
  try {
    const expertId = await expertIdForUser(pool, req.user.id);
    const template = await pool.query("SELECT * FROM inspection_templates WHERE id=$1 AND expert_id=$2 AND status<>'archived'", [req.params.id, expertId]);
    ...
    const created = await pool.query("INSERT INTO inspection_reports (template_id,template_version_id,expert_id,service_request_id,title,status,values_jsonb) VALUES ($1,$2,$3,$4,$5,'draft','{}'::jsonb) RETURNING *", [template.rows[0].id, version.rows[0].id, expertId, serviceRequestId, title]);
...
export const listReports = async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT r.*,t.title AS template_title,u.full_name AS consultant_name,u.email AS consultant_email,v.version_number FROM inspection_reports r JOIN inspection_templates t ON t.id=r.template_id JOIN inspection_template_versions v ON v.id=r.template_version_id JOIN experts e ON e.id=r.expert_id JOIN users u ON u.id=e.user_id${expertId ? " WHERE r.expert_id=$1" : ""} ORDER BY r.updated_at DESC`, expertId ? [expertId] : []);
...
export const createPhotoUploadUrl = async (req, res) => {
  try {
    ...
    const extension = path.extname(clean(req.body.fileName)).toLowerCase(); const key = `inspection-reports/experts/${report.expert_id}/reports/${report.id}/photos/${field.fieldKey}/${crypto.randomUUID()}${extension}`;
...
export const registerPhoto = async (req, res) => {
  try {
    const report = await reportForAccess(pool, req.params.id, req.user, { ownerOnly: true }); const field = report.fields_jsonb.find((item) => item.fieldKey === req.body?.fieldKey && item.type === "photo");
    const prefix = `inspection-reports/experts/${report.expert_id}/reports/${report.id}/photos/${field?.fieldKey}/`;
...
export const generateReport = async (req, res) => {
  try {
    ...
    const sourceMode = report.layout_jsonb?.extractionMode;
    const sourceBytes = sourceMode === "acroform" ? await readPrivateObject(report.source_s3_key, 10 * 1024 * 1024) : null;
    ...
    const bytes = await generateReportPdf({ sourceMode, sourceBytes, title: report.title, fields: report.fields_jsonb, values: report.values_jsonb, photos, consultant: { full_name: report.consultant_name, email: report.consultant_email }, serviceRequest });
    const key = `inspection-reports/experts/${report.expert_id}/reports/${report.id}/generated/report-v${report.version_number}-${crypto.randomUUID()}.pdf`;
