...
test("mapping sends bounded structured evidence in one strict ZDR request", async () => { let calls = 0; let requestBody; const fetchImpl = async (_url, options) => { calls += 1; requestBody = JSON.parse(options.body); return { ok: true, json: async () => ({ choices: [{ message: { content: JSON.stringify(mappedOutput) } }] }) }; }; const result = await mapFieldsWithOpenRouter(mappingEvidence, { fetchImpl, env: mappingEnv }); const sent = JSON.parse(requestBody.messages[1].content); assert.equal(calls, 1); assert.equal(result.fields.length, 1); assert.equal(sent.chunk.blocks[0].text, "Decking in good condition? Yes No"); assert.equal(requestBody.response_format.type, "json_schema"); assert.equal(requestBody.response_format.json_schema.strict, true); assert.deepEqual(requestBody.provider, { require_parameters: true, zdr: true }); assert.equal(requestBody.max_tokens, 6000); assert.equal(requestBody.temperature, 0); });
...
test("OpenRouter 429 is retried once", async () => { let calls = 0; await assert.rejects(() => mapFieldsWithOpenRouter(mappingEvidence, { fetchImpl: async () => { calls += 1; return { ok: false, status: 429 }; }, env: mappingEnv })); assert.equal(calls, 2); });
