...
test("mapping sends bounded structured evidence in one ZDR schema request", async () => { let calls = 0; let requestBody; const fetchImpl = async (_url, options) => { calls += 1; requestBody = JSON.parse(options.body); return { ok: true, json: async () => ({ choices: [{ message: { content: JSON.stringify(mappedOutput) } }] }) }; }; const result = await mapFieldsWithOpenRouter(mappingEvidence, { fetchImpl, env: mappingEnv }); const sent = JSON.parse(requestBody.messages[1].content); assert.equal(calls, 1); assert.equal(result.fields.length, 1); assert.equal(sent.chunk.blocks[0].text, "Decking in good condition? Yes No"); assert.equal(requestBody.response_format.type, "json_schema"); assert.equal(requestBody.response_format.json_schema.name, "template_document_mapping"); assert.deepEqual(requestBody.provider, { require_parameters: true, zdr: true }); assert.deepEqual(requestBody.reasoning, { effort: "high" }); assert.equal(requestBody.max_tokens, 8192); assert.equal(requestBody.temperature, 0); });
...
test("missing key is rejected and the fallback model has a safe default", async () => { await assert.rejects(() => mapFieldsWithOpenRouter(mappingEvidence, { env: { OPENROUTER_TEMPLATE_MODEL: "test/model" } }), /API key is missing/); let requestBody; await mapFieldsWithOpenRouter(mappingEvidence, { env: { OPENROUTER_API_KEY: "test" }, fetchImpl: async (_url, options) => { requestBody = JSON.parse(options.body); return { ok: true, json: async () => ({ choices: [{ message: { content: JSON.stringify(mappedOutput) } }] }) }; } }); assert.equal(requestBody.model, "deepseek/deepseek-v4-flash:free"); assert.deepEqual(requestBody.reasoning, { effort: "high" }); });
